...
Buffer * b_allocate(short init_capacity, char inc_factor, char o_mode) {
	...
	if (o_mode == 'a' && (byte) inc_factor > 0 && inc_factor < 256) {
		buff->mode = 1;
		buff->inc_factor = (byte)inc_factor;
	}else if (o_mode == 'm' && inc_factor > 0 && inc_factor < 101) {
		buff->mode = -1;
...
pBuffer b_addc(pBuffer const pBD, char symbol) {
	...
	if (b_isfull(pBD)==1) {
		...
		if (b_mode(pBD) == 1) {
			if (b_capacity(pBD) == MAX_BUFF)
				return NULL;
			if (b_capacity(pBD) + (unsigned short)b_incfactor(pBD) < 0)
				return NULL;
			if (b_capacity(pBD) + (unsigned short)b_incfactor(pBD) > MAX_BUFF)
				pBD->capacity = MAX_BUFF;
			else
				pBD->capacity = b_capacity(pBD) + (unsigned short)b_incfactor(pBD);
		}
		if (b_mode(pBD) == (-1)) {
			if (b_capacity(pBD) == MAX_BUFF)
				return NULL;
			space = (MAX_BUFF) - b_capacity(pBD);
			newinc = space * (unsigned short)b_incfactor(pBD) / 100;
			if ((unsigned short)b_capacity(pBD) + (unsigned short)newinc > MAX_BUFF) {
				pBD->capacity = MAX_BUFF;
			}
			else
				pBD->capacity = b_capacity(pBD) + newinc;
		}
		pBD->cb_head = realloc(pBD->cb_head, b_capacity(pBD));
		if (pBD->cb_head == NULL)
			return NULL;
...
size_t b_incfactor(Buffer * const pBD) {
	if (!pBD)
		return 0x100;
	return (size_t)(byte)pBD->inc_factor;
}
/********************************************************************
...
int b_load(FILE * const fi, Buffer * const pBD) {
	...
	if (fi == NULL)
		return LOAD_FAIL;
	while (1) {
		thischar = (char)fgetc(fi);
		if (feof(fi))
			break;
		if (b_addc(pBD, (unsigned char)thischar) == NULL) {
			ungetc(thischar, fi);
...
int b_eob(Buffer * const pBD) {
	if (!pBD)
		return RT_FAIL_1;
	return pBD->flags & CHECK_EOB; /*0000 0000 0000 0010*/
}
/********************************************************************
...
Buffer * b_compact(Buffer * const pBD, char symbol) {
	...
	pBD->cb_head[pBD->addc_offset-1] = (unsigned char)symbol;
